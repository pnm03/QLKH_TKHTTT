...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, XMarkIcon, TagIcon, CurrencyDollarIcon, SwatchIcon, ArchiveBoxIcon, FunnelIcon, ChevronDownIcon, ViewColumnsIcon, ListBulletIcon, ChevronRightIcon } from '@heroicons/react/24/outline'
...
export default function ProductSearchPage() {
  ...
  const getProductsByCategory = (categoryId: number) => {
    return filteredProducts.filter(product => product.category_id === categoryId)
  }
        ...
        <div className="p-4 border-b border-gray-200">
          <form onSubmit={handleSearch} className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 rounded-md shadow-sm">
              <div className="relative flex-grow focus-within:z-10 min-w-[200px]">
              ...
              <button
                ...
                className={`relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-gray-50 hover:bg-gray-100 shadow-sm`}
              >
                {viewMode === 'grid' ? (
                  <>
                  ...
                  <>
                    <ViewColumnsIcon className="h-5 w-5 mr-1" aria-hidden="true" />
                    Xem dạng lưới
                  </>
                )}
              </button>
              ...
              <button
                type="submit"
                className={`relative inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white shadow-sm ${getThemeClasses(themeColor, 'bg')}`}
              >
                Tìm
              </button>
            </div>
              ...
              <div className="space-y-4">
                {categories.length > 0 ? (
                  categories.map(category => {
                    ...
                    if (categoryProducts.length === 0 && (searchTerm || colorFilter !== 'all' || sizeFilter !== 'all' || priceFilter !== 'all')) {
                      return null
                    }
                ...
                {categories.length > 0 && filteredProducts.length === 0 && (searchTerm || colorFilter !== 'all' || sizeFilter !== 'all' || priceFilter !== 'all') && (
                  <div className="py-10 text-center">
                    <p className="text-gray-500">Không tìm thấy sản phẩm nào phù hợp.</p>
                  </div>
                )}
